...
for led in leds:
   ...
   if roll!=0:
    ...
    if roll!=0:
        m.rotate([0.5, 0, 0], -roll)
    if pitch!=0:
        m.rotate([0.0, 0.5, 0], -pitch)
    if yaw!=0:
       m.rotate([0.0, 0, 0.5], -yaw)
    m.translate([px-x/2,py-y/2,pz+z/2])
    ...
    #scale = numpy.concatenate([m.points for m in meshes]).flaten(-1)
    #scale = numpy.concatenate([mesh.points]).flaten(-1)
    #axes.auto_scale_xyz(scale, scale, scale)

    # Show the plot to the screen
    #scale = m.points.flaten(-1)
    #axes.auto_scale_xyz(1, 1, 1)
